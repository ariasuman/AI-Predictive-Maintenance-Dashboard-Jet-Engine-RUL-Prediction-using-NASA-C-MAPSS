...
const SENSOR_LABELS = [
  ...
  "s11 (Bleed enthalpy)",   "s12 (Req fan speed)",   "s13 (Corr fan speed)",
  "s14 (HPT coolant bleed)","s15 (LPT coolant bleed)","s17 (Bypass ratio)",
  "s20 (Burner fuel-air)",  "s21 (Demanded fan spd)",
];
...
const card = {
  ...
  padding: "24px", boxShadow: "0 4px 24px rgba(0,0,0,0.4)",
};
...
export default function Predict() {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      if (res.data.error) {
        ...
        const rul = res.data.rul;
        if (rul == null || isNaN(rul)) {
          setError("Model returned invalid prediction. Check sensor values.");
        } else {
          setResult(parseFloat(rul.toFixed(2)));
        }
      }
    } catch {
      setError("Cannot reach backend. Ensure Flask is running on port 5000.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadCSV = () => {
    ...
    const rows = [
      ...
      ["Health Status", getHealth(result)],
    ];
  ...
  const health = result !== null ? getHealth(result) : null;
        ...
        <div style={card}>
          ...
          <div style={{ display: "flex", gap: "12px", marginTop: "20px" }}>
            ...
            {result !== null && (
              <button
                onClick={downloadCSV}
                style={{ padding: "12px 18px", background: "#0f172a", border: "1px solid #334155", borderRadius: "10px", color: "#94a3b8", fontSize: "13px", cursor: "pointer" }}
              >
                ⬇ CSV
              </button>
            )}
            ...
            {result !== null ? (
              <>
                ...
                <div style={{
                  ...
                  <p style={{ fontSize: "32px", fontWeight: 700, color: healthColor[health] }}>{result}</p>
